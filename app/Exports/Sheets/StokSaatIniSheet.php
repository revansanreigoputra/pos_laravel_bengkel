...
class StokSaatIniSheet implements FromCollection, WithHeadings, WithTitle, WithStyles, ShouldAutoSize, WithEvents
{
    protected $startDate;
    protected $endDate;
    ...
    {
        $endDate = $this->endDate ? Carbon::parse($this->endDate)->endOfDay() : now();
        $spareparts = Sparepart::with(['category', 'purchaseOrderItems'])->get();

        $counter = 1;
        return $spareparts->map(function ($item) use ($endDate, &$counter) {
            ...
            return [
                ...
                'Stok Tersedia' => $totalMasuk - $totalKeluar,
            ];
        });
    }
    ...
    {
        return ['No', 'Nama Sparepart', 'Kategori', 'Stok Tersedia'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Title
                $sheet->setCellValue('A1', 'LAPORAN STOK SPAREPART - STOK SAAT INI');
